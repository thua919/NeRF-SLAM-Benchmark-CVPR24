...
def evaluate_ate(first_list, second_list, plot="", _args="",frame_id=0,final_flag = False):
    # parse command line
    ...
    if (args.plot and frame_id%500==0 and frame_id!=0) or (final_flag == True):
        import matplotlib
        matplotlib.use('Agg')
...
def evaluate(poses_gt, poses_est, plot, frame_id=0,final_flag = False):
    ...
    results = evaluate_ate(poses_gt, poses_est, plot, frame_id=frame_id,final_flag = final_flag)
    return results
...
def pose_evaluation(poses_gt, poses_est, scale, path_to_save, i, final_flag = False,img='pose', name='output.txt'):
    N = len(poses_est)
    ...
    results = evaluate(poses_gt, poses_est, plot=plt_path, frame_id=i, final_flag = final_flag)
    results['Name'] = i
