...
from src.NeRFs.encoder import get_encoder, get_pos_embed
from src.NeRFs.decoder import get_decoder
...
class NeRFs(nn.Module):
    def __init__(self, config, bound_box, device):
        ...
        #rebuttal
        self.rebut = 'sampling' if self.config['rebut']['sampling'] else \
            'loss' if self.config['rebut']['loss']  else \
            'keyframe' if self.config['rebut']['keyframe'] else \
            False
        ...
        self.share_encoder = self.config['NeRFs']['tricks']['share_encoder']
        
        if self.config['NeRFs']['F']['choice_MLP']:
            self.encoder = 'freq'
            # get embed functions
            self.pos_embed_freq_fn, self.pos_chanl_freq = get_pos_embed(self.encoder)
        else:
            self.encoder = 'dense' if self.config['NeRFs']['F']['choice_Dense_Grid'] else \
                        'hash' if self.config['NeRFs']['F']['choice_Sparse_Grid'] else \
                        'tri_plane' if self.config['NeRFs']['F']['choice_Tri_Plane'] else \
                        'tensorf' 
            
            if self.encoder == 'dense' or self.encoder == 'hash':
                # get encoder functions
                self.geo_embed_fn, self.geo_chanl = get_encoder(self.encoder, res_level=self.res_level, implicit_dim=self.implicit_dim, 
                ...
                if not self.share_encoder:
                    self.app_embed_fn, self.app_chanl = get_encoder(self.encoder, res_level=self.res_level, implicit_dim=self.implicit_dim, 
                                            resolution_fine=self.app_resolution_fine, resolution_coarse=self.app_resolution_coarse)
                    self.app_embed_fn.to(self.device)
                else:
                    self.app_embed_fn = None
                    self.app_chanl = None
            elif self.encoder == 'tri_plane' or self.encoder == 'tensorf': 
                # get encoded features
                self.geo_embed, _ = get_encoder(self.encoder, res_level=self.res_level, implicit_dim=self.implicit_dim, 
                                            resolution_fine=self.geo_resolution_fine, resolution_coarse=self.geo_resolution_coarse)
                if not self.share_encoder:
                    self.app_embed, _ = get_encoder(self.encoder, res_level=self.res_level, implicit_dim=self.implicit_dim, 
                                            resolution_fine=self.app_resolution_fine, resolution_coarse=self.app_resolution_coarse)
                else:
                    self.app_embed = None
        ...
        if self.share_encoder:
            geo_feat_dim = self.config['NeRFs']['tricks']['geo_app_connect_feat_dim']
        else:
            geo_feat_dim = 0
        
        if self.encoder == 'freq':
            input_ch = self.pos_chanl_freq
            self.decoder = get_decoder(encoder=self.encoder,device = self.device ,bound=self.bounding_box, 
                                       config = self.config, input_ch = input_ch, geo_feat_dim = geo_feat_dim,beta = self.beta)
            # usage: raw = self.decoder(query_points, self.pos_embed_freq_fn)
        elif self.encoder == 'dense' or self.encoder == 'hash':
            input_ch = self.geo_chanl + self.pos_chanl
            self.decoder = get_decoder(encoder=self.encoder,device = self.device ,bound=self.bounding_box, 
            ...
            input_ch = self.implicit_dim*self.res_level + self.pos_chanl
            self.decoder = get_decoder(encoder=self.encoder,device = self.device ,bound=self.bounding_box, 
                                       config = self.config, input_ch = input_ch, pos_ch=self.pos_chanl,
                                        geo_feat_dim = geo_feat_dim, hidden_dim=32, num_layers=2, beta = self.beta)
        ...
        if self.encoder == 'freq':
            self.optimizer = torch.optim.Adam(self.decoder.parameters(), lr=self.map_lr)
        elif self.encoder == 'dense' or self.encoder == 'hash':
            # a lot of parameter unset for faire comparison
            ...
            #    trainable_parameters.append({'params': self.model.embed_fn_color.parameters(), 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed']})
        
            trainable_parameters = [{'params': self.decoder.parameters()},
                                    {'params': self.geo_embed_fn.parameters()}]
            if not self.share_encoder:
                trainable_parameters.append({'params': self.app_embed_fn.parameters()})
            ...
            for planes in [planes_xy, planes_xz, planes_yz]:
                for i, plane in enumerate(planes):
                    ...
                    planes[i] = plane
            if not self.share_encoder:
                c_planes_xy, c_planes_xz, c_planes_yz = self.app_embed
                ...
                self.optimizer = torch.optim.Adam([{'params': self.decoder.parameters(), 'lr':self.map_lr},
                                                    ...
                                                    {'params': c_planes_para, 'lr':self.map_lr}])         
            else:
                self.optimizer = torch.optim.Adam([{'params': self.decoder.parameters(), 'lr':self.map_lr},
                                                    {'params': planes_para, 'lr':self.map_lr}])                
            ...
            for planes in [planes_xy, planes_xz, planes_yz]:
                for i, plane in enumerate(planes):
                    ...
                    planes_para.append(plane)
                    planes[i] = plane
            for lines in [lines_z, lines_y, lines_x]:
                for i, line in enumerate(lines):
                    line = nn.Parameter(line)
                    lines_para.append(line)
                    lines[i] = line
                    
            if not self.share_encoder:
                c_planes_xy, c_planes_xz, c_planes_yz, c_lines_z, c_lines_y, c_lines_x = self.app_embed
                for c_planes in [c_planes_xy, c_planes_xz, c_planes_yz]:
                    for i, c_plane in enumerate(c_planes):
                        c_plane = nn.Parameter(c_plane)
                        c_planes_para.append(c_plane)
                        c_planes[i] = c_plane
                for c_lines in [c_lines_z, c_lines_y, c_lines_x]:
                    for i, c_line in enumerate(c_lines):
                        c_line = nn.Parameter(c_line)
                        c_lines_para.append(c_line)
                        c_lines[i] = c_line
                        
                self.optimizer = torch.optim.Adam([{'params': self.decoder.parameters(), 'lr':self.map_lr},
                                                    {'params': planes_para, 'lr':self.map_lr},
                                                    {'params': lines_para, 'lr':self.map_lr},
                                                    {'params': c_planes_para, 'lr':self.map_lr},
                                                    {'params': c_lines_para, 'lr':self.map_lr}])     
            else:
                self.optimizer = torch.optim.Adam([{'params': self.decoder.parameters(), 'lr':self.map_lr},
                                                    {'params': planes_para, 'lr':self.map_lr},
                                                    {'params': lines_para, 'lr':self.map_lr}])     
    ...
    def sdf_losses(self, sdf, z_vals, gt_depth):
        ...
        # co-slam strategy
        if self.rebut == 'loss':
            fs_loss = torch.mean(torch.square(sdf[front_mask] - torch.ones_like(sdf[front_mask])* self.truncation))
    ...
    def render_rays(self, rays_o, rays_d, target_d = None, target_c = None, render_img=False, tracking=False):
        ...
        if self.encoder == 'freq':
            raw = self.decoder(pts, self.pos_embed_freq_fn)
        elif self.encoder == 'dense' or self.encoder == 'hash':
            raw = self.decoder(pts, self.pos_embed_fn, self.geo_embed_fn, self.app_embed_fn)
        elif self.encoder == 'tri_plane':
            raw = self.decoder(pts, self.pos_embed_fn, self.geo_embed, self.app_embed)
        elif self.encoder == 'tensorf':
            planes_xy, planes_xz, planes_yz, lines_z, lines_y, lines_x = self.geo_embed
            all_geo_planes = planes_xy, planes_xz, planes_yz
            all_geo_lines = lines_z, lines_y, lines_x
            if self.share_encoder: 
                raw = self.decoder(pts, self.pos_embed_fn, all_geo_planes= all_geo_planes , all_geo_lines=all_geo_lines)
                all_c_planes = None
                all_c_lines = None
            else:
                ...
                raw = self.decoder(pts, self.pos_embed_fn, all_geo_planes, all_c_planes , all_geo_lines, all_c_lines)
